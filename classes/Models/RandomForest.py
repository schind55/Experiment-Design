...
class RF:
    ...
    def run(self):
        ...
        max_depth = [10,100]
        n_estimators = [50,100]
        hyperparameters = dict(criterion=criterion,n_estimators=n_estimators,max_depth=max_depth)

        # Create grid search using 5-fold cross validation
        clf = GridSearchCV(rfc, hyperparameters, cv=self.fold, verbose=0)

        best_model = clf.fit(self.X_train, self.y_train)

        # View best hyperparameters
        criterion_value = 'Best criterion: ' + str(best_model.best_estimator_.get_params()['criterion'])
        ...
        obj = pd.Series({'model':'randomforest','content':self.content,'accuracy':accuracy,'f1-score':f1score})
