...
class MNB:
    ...
    def run(self):
        ...
        clf = GridSearchCV(mnb, hyperparameters, cv=self.fold, verbose=0)

        best_model = clf.fit(self.X_train, self.y_train)

        # View best hyperparameters
        alpha_value = 'Best alpha(smoothing): ' + str(best_model.best_estimator_.get_params()['alpha'])
        ...
        obj = pd.Series({'model':'multinomial_nb','content':self.content,'accuracy':accuracy,'f1-score':f1score})
