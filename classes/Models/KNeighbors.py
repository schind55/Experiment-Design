...
class KN:
    ...
    def run(self):
        ...
        #distancemetrics
        hyperparameters = dict(algorithm=algorithm, weights=weights, n_neighbors=n_neighbors)

        # Create grid search using 5-fold cross validation
        clf = GridSearchCV(knc, hyperparameters, cv=self.fold, verbose=0)

        best_model = clf.fit(self.X_train, self.y_train)

        # View best hyperparameters
        n_neighbors_value = 'Best n_neighbors: ' + str(best_model.best_estimator_.get_params()['n_neighbors'])
        weights_value = 'Best weights: ' + str(best_model.best_estimator_.get_params()['weights'])
        algorithm_value = 'Best algorithm: ' + str(best_model.best_estimator_.get_params()['algorithm'])
        ...
        obj = pd.Series({'model':'kneighbors','content':self.content,'accuracy':accuracy,'f1-score':f1score})
