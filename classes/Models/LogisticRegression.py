...
class LR:
    ...
    def run(self):
        # Logistic Regression
        logistic = LogisticRegression(random_state=500)
        ...
        hyperparameters = dict(C=C, penalty=penalty,tol=tol,solver=solver,multi_class=multi_class)

        # Create grid search using 5-fold cross validation
        clf = GridSearchCV(logistic, hyperparameters, cv=self.fold, verbose=0)
        ...
        multi_class_value = 'Best multi_class: ' + str(best_model.best_estimator_.get_params()['multi_class'])
        ...
        obj = pd.Series({'model':'logistic','content':self.content,'accuracy':accuracy,'f1-score':f1score})
