...
class MLP:
    ...
    def run(self):
        # Multi layer perceptron
        mlpc = MLPClassifier(random_state=500)
        ...
        tol = [1e-5,1e-3]
        hyperparameters = dict(alpha=alpha, solver=solver,hidden_layer_sizes=hidden_layer_sizes,activation=activation,tol=tol)

        # Create grid search using 5-fold cross validation
        clf = GridSearchCV(mlpc, hyperparameters, cv=self.fold, verbose=0)

        best_model = clf.fit(self.X_train, self.y_train)

        # View best hyperparameters
        alpha_value = 'Best alpha: ' + str(best_model.best_estimator_.get_params()['alpha'])
        solver_value = 'Best solver: ' + str(best_model.best_estimator_.get_params()['solver'])
        hidden_layers = 'Best hidden_layer_sizes: ' + str(best_model.best_estimator_.get_params()['hidden_layer_sizes'])
        activation_fn = 'Best solver: ' + str(best_model.best_estimator_.get_params()['activation'])
        ...
        obj = pd.Series({'model':'mlp','content':self.content,'accuracy':accuracy,'f1-score':f1score})
        ...
        for i in range(prob.shape[0]):
            ...
            proba_6.append(prob[i][5])
            proba_7.append(prob[i][5])
            proba_8.append(prob[i][5])
